...
;; don't open a backtrace buffer if there's an error (it's annoying)
...
#+begin_src emacs-lisp
(setq org-babel-octave-shell-command "/Applications/Octave.app/Contents/Resources/usr/bin/octave -q")
#+end_src
...
(require 'ggtags)
(define-key ggtags-navigation-map (kbd "<return>") nil)
(define-key ggtags-navigation-map (kbd "C-c C-c") 'ggtags-navigation-mode-done)
...
** ibuffer
#+begin_src emacs-lisp
(defun akt/ibuffer-projectile ()
"First generates ibuffer groups by Projectile project, then calls ibuffer"
(interactive)
...
#+begin_src emacs-lisp
(define-key global-map (kbd "C-x C-b") 'akt/ibuffer-projectile)
#+end_src
** Org Mode
#+BEGIN_SRC emacs-lisp
